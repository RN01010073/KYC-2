...
def ensure_schema():
    """Ensure newly added columns exist in Supabase/PostgreSQL tables."""
    new_pending_cols = [
        ("current_address_proof_path", "TEXT"),
    ]
    new_app_cols = [
        ("same_address", "BOOLEAN DEFAULT TRUE"),
        ("curr_address_line1", "TEXT"),
        ("curr_address_line2", "TEXT"),
        ("curr_city", "VARCHAR"),
        ("curr_state", "VARCHAR"),
        ("curr_pin", "TEXT"),
        ("curr_country", "VARCHAR DEFAULT 'India'"),
        ("current_address_proof_path", "VARCHAR"),
        ("digilocker_photo_b64", "TEXT"),
        ("name_match_score", "INTEGER DEFAULT 0"),
        ("perm_address_match", "BOOLEAN"),
        ("curr_address_match", "BOOLEAN"),
        ("utility_bill_date", "VARCHAR"),
        ("bill_recency_valid", "BOOLEAN"),
        ("face_matched", "BOOLEAN"),
        ("face_match_score", "INTEGER DEFAULT 0"),
    ]
    try:
        with engine.connect() as conn:
            for col, col_type in new_pending_cols:
                try:
                    conn.execute(text(f"ALTER TABLE pending_kyc_sessions ADD COLUMN IF NOT EXISTS {col} {col_type};"))
                    conn.commit()
                except Exception as e:
                    print(f"Schema check on pending col {col}: {e}")
            for col, col_type in new_app_cols:
                try:
                    conn.execute(text(f"ALTER TABLE kyc_applications ADD COLUMN IF NOT EXISTS {col} {col_type};"))
                    conn.commit()
                except Exception as e:
                    print(f"Schema check on app col {col}: {e}")
    except Exception as e:
